...
// Add to _msg_ its own crc32
// Use -mcpu=cortex-a53+crc to enable native CRC instruction on ARM
...
// exec the div/mod box
// _v0_ and _v1_ must be aligned
...
// Rncrypt the first 128 bits of the hash using the last 128 bits as the key
...
static PyObject *rainforest_gethash(PyObject *self, PyObject *args)
{
    char *output;
    PyObject *value;
...
#if PY_MAJOR_VERSION >= 3
    RainforestHash((char *)PyBytes_AsString((PyObject*) input), output, length);
#else
    RainforestHash((char *)PyString_AsString((PyObject*) input), output, length);
#endif
    Py_DECREF(input);
#if PY_MAJOR_VERSION >= 3
...
static PyMethodDef RainforestHashMethods[] = {
    { "get_hash", rainforest_gethash, METH_VARARGS, "Returns result of Rainforest hash" },
    { NULL, NULL, 0, NULL }
};
...
PyMODINIT_FUNC PyInit_rainforest(void) {
    return PyModule_Create(&RainforestHashModule);
}

#else

PyMODINIT_FUNC initrainforest(void) {
    (void) Py_InitModule("rainforest", RainforestHashMethods);
}
#endif
